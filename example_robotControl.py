...
def set_q9(t):
    T = 10
    if t < T:
        return 10*(1-np.cos(t/T*np.pi/2))
    else:
        return 10
def set_q2(t):
    T = 1
...
tennis_robot = my_world.scene.add(Robot(prim_path="/World/Robot/tennis_robot/chair_base", name="robot1"))
tennis_robot.set_default_state(position=np.array([0.0, 0.0, 0.38189]) / get_stage_units())
...
art = dc.get_articulation("/World/Robot/tennis_robot/chair_base")
q9 = dc.find_articulation_dof(art, "Revolute_9")
q8 = dc.find_articulation_dof(art, "Revolute_8")
